...
test("uses environment overrides for Signal API URL", function()
  local original_getenv = os.getenv
  local function fake_getenv(key)
    if key == "PORT" then
      return "18181"
    end
    if key == "SIGNAL_API_HOST" then
  ...
  local ok, err = pcall(function()
    ...
    package.loaded["src.seerr-notification.constants"] = nil
    local constants = require("src.seerr-notification.constants")
    assert(constants.SIGNAL_API_PORT == "18181")
    assert(constants.SIGNAL_API_HOST == "http://example.local")
    assert(constants.SIGNAL_API_URL == "http://example.local:18181")
  end)
